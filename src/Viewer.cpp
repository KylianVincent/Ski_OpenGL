    ...
    m_guidingRenderable{NULL}
{
    ...
    // Add-on for GL3.30 compatibility: create a VAO manually
...
void Viewer::draw()
{
    ...
    for (ShaderProgramPtr prog : m_programs) {
        prog->bind();
        // LOG(info, "draw, shader " << prog->programId());
        bool success;
        success = DirectionalLight::sendToGPU(prog, m_directionalLight);
        success &= SpotLight::sendToGPU(prog, m_spotLights);
        success &= PointLight::sendToGPU(prog, m_pointLights);
        // if (!success)
        //     LOG(error, "error sending lights to shader " << prog->programId());
    }
