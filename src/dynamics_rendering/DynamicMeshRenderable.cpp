...
DynamicMeshRenderable::DynamicMeshRenderable(ShaderProgramPtr shaderProgram,
        const std::string& meshFilename, const std::string& textureFilename,
        DynamicMeshPtr mesh) :
    HierarchicalRenderable(shaderProgram),
    m_mesh(mesh),
    m_pBuffer(0), m_nBuffer(0), m_iBuffer(0),
    m_tBuffer(0), m_texId(0)
{
    // Load the geometry (vertices, normals, indices) and texture coordinates
    read_obj(meshFilename, m_positions, m_indices, m_normals, m_texCoords);
...
void DynamicMeshRenderable::do_draw()
{
    const glm::vec3& pPosition = m_mesh->getPosition();
    glm::mat4 translate = glm::translate(glm::mat4(1.0), glm::vec3(pPosition));
    setParentTransform(translate);
    // std::cout << "(" << pPosition[0] << " ," << pPosition[1] << " ," << pPosition[2] << ")" << std::endl;
...
void DynamicMeshRenderable::do_animate(float time)
{
}

void DynamicMeshRenderable::setMaterial(const MaterialPtr& material)
{
    m_material = material;
}
