...
void teachers::getUnitCube(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2>& texCoords)
{
    //Face 1
...
void teachers::getUnitIndexedCube(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::ivec3>& indices)
{
    positions.push_back(glm::vec3(-0.5, -0.5, -0.5));
    positions.push_back(glm::vec3(0.5, -0.5, -0.5));
    positions.push_back(glm::vec3(0.5, 0.5, -0.5));
...
void teachers::getUnitPlane(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2> &texCoords)
{
...
void teachers::getUnitCylinderTextured(std::vector<glm::vec3> &positions, std::vector<glm::vec3> &normals, std::vector<glm::vec2> &texCoords, const unsigned int &slices)
{
    size_t number_of_triangles = slices * 4;
    size_t number_of_vertices = number_of_triangles * 3;
    float angle_step = 2.0 * M_PI / double( slices );
            ...
            angle += angle_step, cos = std::cos( angle ), sin = std::sin( angle) )
    {
        ...
        //Face du haut
        texCoords[ voffset + 0] = glm::vec2(0.5, 1);
        texCoords[ voffset + 1] = glm::vec2(0, 0);
        texCoords[ voffset + 2] = glm::vec2(0, 1);

        //Faces la térales
        texCoords[ voffset + 3] = glm::vec2(0.5 + asin(sin)/M_PI, 1);
        texCoords[ voffset + 4] = glm::vec2(0.5 + asin(previous_sin)/M_PI, 1);
        texCoords[ voffset + 5] = glm::vec2(0.5 + asin(previous_sin)/M_PI, 0);

        texCoords[ voffset + 6] = glm::vec2(0.5 + asin(sin)/M_PI, 1);
        texCoords[ voffset + 7] = glm::vec2(0.5 + asin(previous_sin)/M_PI, 0);
        texCoords[ voffset + 8] = glm::vec2(0.5 + asin(sin)/M_PI, 0);

        //Face du bas
        texCoords[ voffset + 9] = glm::vec2(0.5, 1);
        texCoords[ voffset + 10] = glm::vec2(0, 0);
        texCoords[ voffset + 11] = glm::vec2(0, 1);

    }
}

void teachers::getUnitSphere(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, const unsigned int& strips, const unsigned int& slices)
{
    double radius=1.0;
...
void teachers::getUnitSphereTextured(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2>& texCoords, const unsigned int& strips, const unsigned int& slices)
{
    double radius=1.0;
    glm::vec3 center(0.0,0.0,0.0);
    ...
    for(int i=0; i<slices; ++i)
    {
        for(int j=0; j<strips; ++j)
        {
            ...
            glm::vec3 d = glm::normalize(vTriangles[0] - center);
            texCoords[i*strips*6 + 6*j +0] = glm::vec2(0.5 + atan2(d[1], d[0])/(2*M_PI), 0.5 + asin(d[2])/M_PI);
            d = glm::normalize(vTriangles[1] - center);
            texCoords[i*strips*6 + 6*j +1] = glm::vec2(0.5 + atan2(d[1], d[0])/(2*M_PI), 0.5 + asin(d[2])/M_PI);
            d = glm::normalize(vTriangles[2] - center);
            texCoords[i*strips*6 + 6*j +2] = glm::vec2(0.5 + atan2(d[1], d[0])/(2*M_PI), 0.5 + asin(d[2])/M_PI);
            ...
            d = glm::normalize(vTriangles[0] - center);
            texCoords[i*strips*6 + 6*j +3] = glm::vec2(0.5 + atan2(d[1], d[0])/(2*M_PI), 0.5 + asin(d[2])/M_PI);
            d = glm::normalize(vTriangles[1] - center);
...
void teachers::getTorus(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, const float& a, const float& c, const unsigned int& strips, const unsigned int& slices)
{
    unsigned int vertexNumber = slices*strips*6;
    positions.resize(vertexNumber);
    normals.resize(vertexNumber);
...
void teachers::getUnitCone(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, const unsigned int& strips, const unsigned int& slices)
{
    double height = 1.0;
    double radius=1.0;
    glm::vec3 center(0.0,0.0,0.0);
...
void teachers::getUnitConeTextured(std::vector<glm::vec3>& positions, std::vector<glm::vec3>& normals, std::vector<glm::vec2>& texCoords, const unsigned int& strips, const unsigned int& slices)
{
    double height = 1.0;
    double radius=1.0;
    glm::vec3 center(0.0,0.0,0.0);
    ...
    for(int i=0; i<slices; ++i)
    {
        ...
        texCoords[counter+3*i+0] = glm::vec2(1, 0);
        texCoords[counter+3*i+1] = glm::vec2(1, 1);
        texCoords[counter+3*i+2] = glm::vec2(0, 0);
